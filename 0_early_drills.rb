...
# 6. Now instead of setting the name variable as a hard coded string, Get the name from the command line using `gets`.
# Test out the difference between using `gets.chomp` and `gets.strip` (you will likely need to use either `.chomp` or `.strip` every time you invoke `gets`).

# 7. Our admin is on Level 8. The FastTrack classroom is on Level 12. The GenTech classroom is on Level 13. The Cyber Bootcamp is on Level 13 also. Have the user let you know which floor they are on, and tell them which aspects of the Coder business they can visit at that floor. 
# Now turn it around and have them enter which room they are in (FastTrack, Cyber, etc), and let them know which floor they are on.
...
# If they drink three coffees in a day respond with "Hmm.. it's getting a bit much".
...
# 10. The user will enter a 24 hour based time as a float (so 5:38pm would be 17.38). If the user enters a number that is less than 12 puts out "Morning"; if the number entered is exactly 12, respond with "Midday"; if number is greater than 12 but less than or equal to 13 puts "Lunch"; if the number is greater than 13, but less than 17, puts "Afternoon"; if the number entered is less than or equal to 21 but more than or equal to 17 puts "Evening"; if number is greater than 21 but less than or equal to 24 puts "Time for bed". If they enter 0 or 24, respond with "Midnight". If the time isn't in the 24 hour range puts "Not a valid 24 hour time".
